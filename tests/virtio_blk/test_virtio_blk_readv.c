...
void mock_bdev_queue_readv(void *self, uint16_t tag, struct iovec *iov, int iovcnt, off_t offset, bdev_callback_t cb, void *ctx) {
...
int mock_bdev_queue_poll(void *self) {
    ...
    for (size_t i = 0; i < queue->iovcnt; i++) {
        memset(queue->iov[i].iov_base, 1, queue->iov[i].iov_len);
    }
